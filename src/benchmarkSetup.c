...
int findBenchmarkNumber() {
  ...
  /* If pingpong or pingping benchmark then broadcast ranks of participating
   * processes */
  if (benchmarkNumber <= LASTPPID) { MPI_Bcast(PPRanks, 2, MPI_INT, 0, comm); }
...
int convertToLowercase(char *convertString) {
  ...
  for (i = 0; i < len; i++) { convertString[i] = tolower(convertString[i]); }
