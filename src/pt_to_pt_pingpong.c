...
#pragma omp parallel for default(none) private(i)                              \
  shared(pingSendBuf, dataSize, sizeofBuffer, globalIDarray)                   \
    schedule(static, dataSize)
       ...
       */
      MPI_Recv(pongRecvBuf, sizeofBuffer, MPI_INT, pongRank, TAG, comm,
               &status);
...
#pragma omp parallel for default(none) private(i)                              \
  shared(pongRecvBuf, finalRecvBuf, dataSize, sizeofBuffer)                    \
    schedule(static, dataSize)
      ...
      MPI_Recv(pingRecvBuf, sizeofBuffer, MPI_INT, pingRank, TAG, comm,
               &status);
...
#pragma omp parallel for default(none) private(i)                              \
  shared(pongSendBuf, pingRecvBuf, dataSize, sizeofBuffer)                     \
    schedule(static, dataSize)
      ...
      MPI_Send(pongSendBuf, sizeofBuffer, MPI_INTEGER, pingRank, TAG, comm);
    }
  }
...
#pragma omp parallel default(none) private(i, repIter, lBound)                 \
  shared(pingRank, pongRank, pingSendBuf, pingRecvBuf)                         \
    shared(pongSendBuf, pongRecvBuf, finalRecvBuf, sizeofBuffer)               \
      shared(dataSize, globalIDarray, comm, status, totalReps, myMPIRank)
  {
...
#pragma omp for nowait schedule(static, dataSize)
         ...
         */
        MPI_Send(&pingSendBuf[lBound], dataSize, MPI_INT, pongRank, myThreadID,
                 comm);

        /* Thread then waits for a message from pong process. */
        MPI_Recv(&pongRecvBuf[lBound], dataSize, MPI_INT, pongRank, myThreadID,
                 comm, &status);
...
#pragma omp for nowait schedule(static, dataSize)
         ...
         * from the ping process.
         */
        MPI_Recv(&pingRecvBuf[lBound], dataSize, MPI_INT, pingRank, myThreadID,
                 comm, &status);
...
#pragma omp for nowait schedule(static, dataSize)
        ...
        MPI_Send(&pongSendBuf[lBound], dataSize, MPI_INT, pingRank, myThreadID,
                 comm);
      }
