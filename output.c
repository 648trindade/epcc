...
int setReportParams(int size, int reps, double time) {
  ...
  if (benchReport.benchNumber <= LAST_PT_PT_ID) {
    ...
    benchReport.bytes =
      size * benchReport.numThreads * sizeInteger * localCommSize;
  } else {
...
int printReport() {
  /* Write to output */
  printf("d %d\t\t%d\t\t   %d\t\t%lf\t%lf\t%s\n", benchReport.dataSize,
         benchReport.bytes, benchReport.numReps, benchReport.benchTime,
         benchReport.timePerRep, benchReport.testOutcome);
