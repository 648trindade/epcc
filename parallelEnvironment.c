...
#pragma omp parallel default(none) shared(numThreads, globalIDarray, myMPIRank)
  {
...
int compareProcNames(int rankA, int rankB) {
  ...
  if (myMPIRank == rankB) {
    ...
    MPI_Recv(recvProcName, MPI_MAX_PROCESSOR_NAME, MPI_CHAR, rankB, TAG, comm,
             &status);
...
void sendProcName(int destNode, int srcNode, char *destProcName) {
  ...
  if (crossCommRank == srcNode) {
    MPI_Send(myProcName, MPI_MAX_PROCESSOR_NAME, MPI_CHAR, destNode, TAG,
             crossComm);
  } else if (crossCommRank == destNode) {
    MPI_Recv(destProcName, MPI_MAX_PROCESSOR_NAME, MPI_CHAR, srcNode, TAG,
             crossComm, &status);
  }
}
...
int crossCommBalance(int nodeA, int nodeB) {
  ...
  /* Master process on nodeA... */
    ...
    MPI_Recv(&otherLocalCommSize, 1, MPI_INT, nodeB, TAG, crossComm, &status);
  ...
  if (myMPIRank == 0) {
    ...
    if ((crossCommRank != nodeA) && (localCommRank != 0)) {
      MPI_Recv(&crossCommRank, 1, MPI_INT, MPI_ANY_SOURCE, TAG, comm, &status);
    }
  }
